...
public class Main extends JFrame {
    ...
    public static void main(String[] args) {
       ...
       /* for (double x : loads){
        ...
        //这里我直接用的绝对路径
        saveAsFile(freeChart, "/Users/ace/Desktop/射频识别/pure aloha/jpg/line.jpg", 600, 400);
    }
    ...
    public double troughput(ArrayList<Double> pkts,double load){
        ...
        for (int i = 1;i< pkts.size()-1;i++){
            double fail = 0.0;
            //pkts[x] - pkts[x-1] > 1 and pkts[x+1] - pkts[x] > 1
            //(pkts.get(i) - pkts.get(i-1))> 1 && (pkts.get(i+1) - pkts.get(i)) > 1
           /* System.out.println(pkts.get(i)*-1);
            System.out.println(pkts.get(i-1)*-1);
            System.out.println(pkts.get(i+1)*-1);
    ...
    public static JFreeChart createChart(CategoryDataset categoryDataset) {
        ...
        JFreeChart jfreechart = ChartFactory.createLineChart("pure aloha仿真", // 标题
